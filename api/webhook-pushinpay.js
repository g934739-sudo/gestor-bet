      ...
      <tr><td style="position:relative;overflow:hidden;border-radius:20px;">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          <tr><td class="cardpad" style="padding:30px 32px 0;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
              <td class="mono" style="font-size:10px;letter-spacing:2.4px;color:rgba(255,203,31,0.85);text-transform:uppercase;">Grivo Bet &middot; Membro</td>
              <td align="right">
  ...
  <tr><td class="px-pad anim4" style="padding:32px 40px 0;" align="center">
    <table role="presentation" cellpadding="0" cellspacing="0" align="center"><tr>
      <td align="center" class="shine" style="border-radius:12px;box-shadow:0 14px 36px rgba(255,203,31,0.3);">
        <a class="btn-a px" href="https://www.grivo.bet/login" style="display:inline-block;padding:19px 42px;font-size:16px;font-weight:700;color:#0a0a0c;letter-spacing:-0.2px;border-radius:12px;">Acessar Grivo Bet  &rarr;</a>
      </td>
    </tr></table>
    <div class="mono" style="font-size:11px;letter-spacing:0.5px;color:#5a5a63;margin-top:16px;">Acesso imediato &middot; funciona no celular e no computador</div>
  </td></tr>
...
<style>
  ...
  @media only screen and (max-width:620px){
    ...
    .cred-cell{display:block!important;width:100%!important;border-right:0!important;}
...
// ─── Monta e-mail de boas-vindas com dados do assinante ──────────────────────────
function emailBoasVindas({ nome, userId, plano, data }) {
  const idMembro = userId ? userId.slice(-4).toUpperCase() : '0000';
  return EMAIL_BOAS_VINDAS_TEMPLATE
    .replace('{{nome_cliente}}', nome)
    .replace('{{id_membro}}', idMembro)
    .replace('{{plano}}', plano)
    .replace('{{data_assinatura}}', data);
}

// ─── Monta e-mail de dados de acesso ───────────────────────────────────────────
...
module.exports = async function handler(req, res) {
  ...
  const dataAssinatura = new Date().toLocaleDateString('pt-BR', { month: 'short', year: 'numeric' });
  const boasVindasHtml = emailBoasVindas({ nome: primeiroNome, userId, plano: planoNome, data: dataAssinatura });
  const dadosAcessoHtml = emailDadosAcesso({ email, senha });

  const emailResults = await Promise.allSettled([
    sendEmail({
      to:      email,
      subject: `Você está dentro, ${primeiroNome}! Bem-vindo ao Grivo Bet`,
      html:    boasVindasHtml,
    }),
    sendEmail({
      to:      email,
